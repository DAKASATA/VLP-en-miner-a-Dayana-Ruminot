...
%metros de altura, 4 metros de ancho y 16 metros de largo.

%Consideraciones:
%El túnel tiene lumianrias colineales entre sí a lo largo del eje X
%(16[m]), se estudia la separación que deben tener las luminarias entre sí
...
Pos_FD = zeros(16, 3);
for i = 1:16                        %en total son 16 puntos y solo avanzará en el eje X
    Pos_FD(i, :) = [i, 2, 2.6];     % Posiciones del fotodetector (x, y, z)
end

FD = [x_j, y_j, z_j];                       % Usar la posición del receptor
P_transmision = 20;                         % Potencia de transmisión de los LEDs
...
% Si hay más de un LED con la potencia máxima, se mostraran las coordenadas adicionales
...
matriz_dist_ecuclidiana = zeros(3, 16);     %matriz de distancias ecuclidiana entre receptor en cada distancia en el eje X y cada uno de los transmisores de posición fija
Precepcion_t = zeros(16, 6);                %en cada una de las posiciones almacena la altura del PD(h_i), la distancia del LED de distancia euclidiana más cercana al PD (d_cercana_i), los impulsos del canal de cana uno de los 3 LEDs (H_LOS_W_t_all(i, j)), t_i que corresponde a la respuesta al impluso de la distancia más cercana
P_recep = zeros(16, 3);                     %vector utilizado para almacenar los resultados de potencia recibida      

H_LOS_W_t_all = zeros(16, 3); % 16 posiciones para el FD y 3 LEDs
...
SNR_Paralelos_por_LED = zeros(16,3);
SNR_MAX_Paralelos = zeros(16,2); %se tienen 2 valores en cada vector por SNR y posición, esta info sirve para gráficas
SNR_MIN_Paralelos = zeros(16,2); %se tienen 2 valores en cada vector por SNR y posición, esta info sirve para gráficas


% Inicialización del vectores para almacenar la potencia con ruido.
Pr_ruido_termico_vec = zeros(16, 3); 
Pr_ruido_disparo_vec = zeros(16, 3); 
Pr_ruido_disparo_y_termico_vec = zeros(16, 3); 
...
Psi = zeros(16, 3);                     % Almacenar valores de Psi para cada combinación i,j
distancia_euclidiana = zeros(16, 3);    % Almacenar distancias euclidianas
HLOS = zeros(16, 3);                    % Almacenar HLOS para cada combinación i,j
Pr_plano_no_paralelos = zeros(16, 3);   % Almacenar potencia por plano no paralelo

% Inicializar vectores de ruido
Pr_ruido_termico_vec_planos_no_paralelos = zeros(16, 3); 
Pr_ruido_disparo_vec_planos_no_paralelos = zeros(16, 3); 
Pr_ruido_disparo_y_termico_vec_planos_no_paralelos = zeros(16, 3); 


% Inicializaar vectores de SNR
SNR_NO_Paralelos_por_LED = zeros(16,3);
SNR_MAX_NO_Paralelos = zeros(16,2); 
SNR_MIN_NO_Paralelos = zeros(16,2);


% Inicialización del vector de potencia más alta por receptor
potencias_por_receptor = zeros(16, 1);
mejor_transmisor = zeros(16, 1);
...
Pos_real = zeros(16, 2);
for i = 1:16 % En total son 16 puntos
    Pos_real(i, :) = [i, 2]; % (x, y)
end
...
d_estimada_no_paralelos = zeros(16, 3);
Pos_est_no_paralelos = zeros(16,2);
error_no_paralelos = zeros(16,1);
...
d_estimada_paralelos = zeros(16, 3);
Pos_est_paralelos = zeros(16,2);
error_paralelos = zeros(16,1);
...
function plotPositions(real_pos, estim_pos)
    ...
    xlabel('X');
    ylabel('Y');
    axis([0 16 0 4]); % Ajustar el rango del gráfico
    axis equal;
    grid on;
